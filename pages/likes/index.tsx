...
import { EMPTY, Observable, catchError, filter, forkJoin, from, map } from "rxjs";
...
export default function Likes() {
  ...
  const getLikedVideosInfo = useCallback(
    ...
    return (
      <Fragment>
        {videoIds?.map((item, index) => {
          return (
            <div
              className={styles.thumbnailPrimaryLoaderTemplateItem}
              key={index}
            >
      ...
      <div className={styles.host}>
        <div className={styles.home}>
          <div className={styles.homeSidebar}>
            ...
            <h1 className={`${styles.homeSidebar__title} mat-display-1`}>
              Liked videos
            </h1>
          </div>
              ...
              ? likedVideos.map((video, videoIndex) => {
                  return (
                    <div key={videoIndex} className={styles.videoItem}>
                      {
                        <div className={styles.videoItem__index}>
                          {videoIndex + 1}
                        </div>
                      }
